...
- (void)buildViews {
    ...
    _m_timeLabel.font          = [UIFont fontWithName:@"AvenirNext-UltraLight" size:20];
...
- (void)loadData:(id)data indexPath:(NSIndexPath *)indexPath {
    ...
    if ([data isMemberOfClass:[TimeModel class]]) {
        ...
        if ([_m_timeLabel.text isEqualToString:@"00:00:00"]) {
//            UIButton * button = [[UIButton alloc]initWithFrame:self.m_timeLabel.frame];
//            button.backgroundColor = [UIColor redColor];
//            [button setTitle:@"点击查看结果" forState:UIControlStateNormal];
//            [self.m_timeLabel removeFromSuperview];
//            [_m_backgroundView addSubview:button];
        }
    }
